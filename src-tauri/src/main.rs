...
struct AppState(
    ...
    std::sync::Mutex<Option<UnboundedSender<bool>>>,
);
...
async fn fetch(
    ...
    headers.insert(
        ...
        "Sunshine RustUI v0.0.1-beta".parse().unwrap(),
    );
    ...
    let result = match result {
        ...
        Err(_error) => {
            return Err("Response failed: ".to_string() + &_error.to_string());
        }
    };
...
async fn stop_sse(state: tauri::State<'_, AppState>) -> Result<String, String> {
    ...
    match state.2.try_lock() {
        Ok(mut send) => match send.as_mut() {
            Some(send) => match send.send(true) {
...
fn main() {
    let client_builder = reqwest::Client::builder().danger_accept_invalid_hostnames(true);

    let cert = read_certificate_file();
    let client = match cert {
        Ok(ref cert) => client_builder
            .add_root_certificate(cert.clone())
            .build()
            .unwrap(),
        Err(ref _err) => client_builder.build().unwrap(),
    };
        ...
        tauri::RunEvent::WindowEvent { label, event, .. } => match event {
            tauri::WindowEvent::CloseRequested { .. } => {
                if label == "main" {
                    ...
                    std::thread::spawn(move || match app_handle.get_window("pin_request") {
                        None => {}
                        Some(window) => {
                            window.close().unwrap();
                        }
